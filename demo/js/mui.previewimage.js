(function($, window) {
	...
	var itemTemplate = '<div class="mui-slider-item mui-zoom-wrapper {{className}}"><div class="mui-zoom-scroller"><img src="{{src}}" data-preview-lazyload="{{lazyload}}" data-original-lazyload="{{real_src}}" style="{{style}}" class="mui-zoom"></div></div>';
	...
	proto.addImage = function(img) {
		...
		if (img.__mui_img_data && img.__mui_img_data.src === src) { //已缓存且图片未变化
			...
			var lazyload = img.getAttribute('data-preview-src'),
				...
				zan= img.getAttribute('data-zan');
				zan_on= img.getAttribute('data-zan-on');
				real_src=img.getAttribute('data-original');
				mid = img.getAttribute('data-mid');
			if (!lazyload) {
				lazyload = src;
			}
			// if (src.indexOf("loading.gif")!=-1) {
			// 	src=real_src;
			// 	// this.refresh(index,this.currentGroup)
			// };
			var imgObj = {
				src: src,
				real_src:real_src,
	...
	proto._loadItem = function(index, isOpening) { //TODO 暂时仅支持img
		...
		var zan_num=document.getElementById('zan_num');
        img_size.innerHTML = itemData.size;
        if (zan_num) {
            zan_num.innerHTML = itemData.zan;
        }
		
		if (itemData.src.indexOf("loading.gif")!=-1) {
			// itemData.src=itemData.real_src;
			// this.refresh(index,this.currentGroup)
			// this._loadItem(index,true)
			this.loadImageReal(imgEl, function() {
			 	// itemData.loaded = true;
			 	console.log(itemData.real_src)
				imgEl.src = itemData.real_src;
		...
		if (itemData.zan_on==true) {
			jQuery(".zan").addClass('zan-on')
		}else{
			jQuery(".zan").removeClass('zan-on')
			jQuery(".add1").removeClass('add-active')
		};
		if (!itemData.loaded && imgEl.getAttribute('data-preview-lazyload')) {
			...
			imgEl.offsetHeight;
			self.loader.classList.remove($.className('active'));
			document.getElementById('view-big').innerHTML="查看原图"
			...
			imgEl.setAttribute('style', '');
			imgEl.offsetHeight;
			document.getElementById('view-big').innerHTML="已加载原图"
		}
	...
	proto.loadImageReal = function(imgEl, callback) {
		var onReady = function() {
			callback && callback.call(this);
		};
		var img = new Image();
		img.onload = onReady;
		img.onerror = onReady;
		img.src = imgEl.getAttribute('data-original-lazyload');
	};
	...
	proto.refresh = function(index, groupArray) {
		...
		for (var i = 0; from < to; from++, i++) {
			...
			itemStr = itemTemplate.replace('{{src}}', itemData.src).replace('{{lazyload}}', itemData.lazyload).replace('{{real_src}}', itemData.real_src).replace('{{style}}', style);
			if (from === index) {
				currentIndex = i;
	...
	proto.open = function(index, group) {
        ...
        if (typeof index === "number") {
            ...
            $.ajax({
                url: "/Ajax/AddBrowses/?id=" + index.getAttribute('v'),
                cache: false,
